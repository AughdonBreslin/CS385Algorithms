...
int main(int argc, char *argv[]) {
    ...
    if(num_values < k){
    	...
    	cerr << "Error: Cannot find smallest element " << k << " with only " << num_values << val << endl;
    	return 1;
    }
    int *arr = new int[num_values]; //can just do &values[0]

    for (int i = 0; i < num_values; i++){
    	arr[i] = values[i];
    }
    // TODO - call the quick_select function and display the result
    cout << "Smallest element " << k << ": " << quick_select(arr, num_values, k);
