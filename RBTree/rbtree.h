	...
	void insert(const iterator &it, const std::pair<K, V> &key_value) {
		...
		while (x != nullptr) {
			...
			if(key < x->key()){
				...
				throw tree_exception("Attempt to insert duplicate key '" +word+ "'.");
			}
		}
		z->parent = y;
		if(y == nullptr){
...
private:
	...
	void delete_tree(Node<K, V> *n) {
		// TODO
		//post-order deletion so we always delete when we leaving for last time
		if(n != nullptr){
			//Has no children
	...
	void insert_fixup(Node<K, V> *z) {
		// TODO
		//basically make sure z isn't root
	...
	int diameter(Node<K, V> *node) const {
		...
		//longest path w root
		//left subtree height + right subtree height + root
		int leftH = height(node->left)+1;
		int rightH = height(node->right)+1;

		//longest path w/o root
		//longest path in left and subtree
		int leftD = diameter(node->left);
		int rightD = diameter(node->right);

		return std::max((leftH + rightH + 1), std::max(leftD,rightD));
	}
	...
	size_t width(Node<K, V> *node, size_t level) const {
		...
		//has right
		if(node->right != nullptr && node->left == nullptr){
			return width(node->right, level - 1);
		}
		//Has both
