 ...
 * Version : 1.0
 * Date    : Sep 9, 2020
 * Description : A student class with a few functions
...
class Student {
public:
	//Constructor that uses an initializer list NEAR CHECK
	Student(string first, string last, float gpa, int id) :
		first_{first}, last_{last}, gpa_{gpa}, id_{id} { }

	// Methods that do not modify member variables should be declared const. CHECK
	...
	void print_info() const {
		cout << full_name() << ", GPA: " << setprecision(2) << fixed << gpa_ << ", ID: " << id_ << endl;
	}
private:
	string first_, last_;
	float gpa_;
...
int main() {
    ...
    if (fails.empty()) {
    	cout << " None" << endl;
    } else { // Otherwise, print each failing student on a separate line.
    	cout << endl;
